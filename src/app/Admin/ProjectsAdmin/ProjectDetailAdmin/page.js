...
export default function ProjectDetailAdmin() {
        ...
        if (res.data && Array.isArray(res.data.value)) {
          setEquipment(
            res.data.value.map((equipment) => ({
              code: equipment._id,
              name: equipment.imageTitle || equipment.image,
  ...
  const getEquipment = (equipmentNo) => {
    // ✅ جدید: مدیریت هر دو حالت رشته و آرایه
    const actualId = Array.isArray(equipmentNo) 
      ? equipmentNo[0] 
      : equipmentNo;
    
    const found = equipment.find((i) => String(i.code) === String(actualId));
    if (found && found.image) {
        return found.image;
    } else {
        console.warn("تصویر یافت نشد یا خالی است برای:", actualId);
        return null;
    }
  };
  ...
  return (
    <MangerAdmin
      ...
      cart={[
        ...
        { value:"tilte", header: "عنوان پروژه" },
            ...
            return (
              <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1 }}>
                {options.map((opt) => {
                  ...
                  return (
                    <Tooltip key={opt.code} title={opt.name}>
                      <Box
                        component="img"
